...
if __name__ == '__main__':
    ...
    #         print ('minDCF - pT = 0.5: %.4f' % bayes_risk.compute_minDCF_binary_slow(SVAL, LVAL, 0.5, 1.0, 1.0))
    #         print ('actDCF - pT = 0.5: %.4f' % bayes_risk.compute_actDCF_binary_fast(SVAL, LVAL, 0.5, 1.0, 1.0))
    #         print ()
    ...
    for kernelFunc in [polyKernel(2, 0), polyKernel(2, 1), rbfKernel(1.0), rbfKernel(10.0)]:
        for eps in [0.0, 1.0]:
            ...
            err = (PVAL != LVAL).sum() / float(LVAL.size)
            print ('Error rate: %.1f' % (err*100))
            print ('minDCF - pT = 0.5: %.4f' % bayes_risk.compute_minDCF_binary_fast(SVAL, LVAL, 0.5, 1.0, 1.0))
            print ('actDCF - pT = 0.5: %.4f' % bayes_risk.compute_actDCF_binary_fast(SVAL, LVAL, 0.5, 1.0, 1.0))
            print ()
